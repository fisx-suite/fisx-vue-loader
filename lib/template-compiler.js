...
function rewriteEleUrl(el, attrNode, name, context) {
    ...
    if (attrNode.name === 'srcset') {
        url = context.htmlParser.srcset2Arr(url);
    }

    attrNode.value = context.urlRewriteProcessor(url, value, context);
    return true;
}
...
module.exports = function (context, opts) {
    ...
    var toTransformEle = opts.transformEle;
    ...
    var compiled = vueCompiler.compile(context.content, compileOpts);
